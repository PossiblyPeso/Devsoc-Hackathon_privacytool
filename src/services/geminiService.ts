...
export const analyzePolicyText = async (text: string): Promise<AnalysisResult> => {
  if (!process.env.API_KEY) {
    throw new Error("Gemini API key not found. Please create a .env file in the project root and add your key as API_KEY=\"YOUR_API_KEY\".");
  }
  ...
  try {
    ...
    if (error instanceof Error) {
        if (error.message.includes("API key not valid")) {
            throw new Error("The provided Gemini API key is not valid. Please check your .env file.");
        }
        throw error;
    }
